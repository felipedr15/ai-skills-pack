...
class CLITests(unittest.TestCase):
    ...
    def test_status_command(self):
        ...
        self.assertEqual(code, 0)
        self.assertIn("AI OS v0.7.0", out)
...
class PackagingTests(unittest.TestCase):
    ...
    def test_package_create(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 0)
            dist = Path(tmp)
            self.assertTrue((dist / "ai-os-0.7.0.zip").is_file())
            self.assertTrue((dist / "ai-os-0.7.0.tar.gz").is_file())
            self.assertTrue((dist / "ai-os-0.7.0.sha256").is_file())
            self.assertTrue((dist / "ai-os-0.7.0-files.txt").is_file())
