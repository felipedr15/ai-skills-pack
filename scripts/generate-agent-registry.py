#!/usr/bin/env python3
"""Build or check the AI OS agent registry (Phase 8, Part 1)."""
...
from orchestration.render import render_agent_registry_md, render_json

ROOT = Path(__file__).resolve().parents[1]
OUT_JSON = ROOT / "generated" / "agent-registry.json"
OUT_MD = ROOT / "generated" / "agent-registry.md"
...
def write_registry(registry: dict) -> None:
    ...
    OUT_MD.write_text(render_agent_registry_md(registry), encoding="utf-8", newline="\n")


def check_registry(registry: dict) -> None:
    if not OUT_JSON.is_file() or not OUT_MD.is_file():
        raise RegistryBuildError("stale generated files: generated/agent-registry.json, generated/agent-registry.md")

    saved = json.loads(OUT_JSON.read_text(encoding="utf-8"))
    if not compare_registries_ignoring_generated_at(registry, saved):
        raise RegistryBuildError("stale generated files: generated/agent-registry.json")

    expected_md = render_agent_registry_md(registry)
    ...
    if _normalize_markdown_timestamp(saved_md) != _normalize_markdown_timestamp(expected_md):
        raise RegistryBuildError("stale generated files: generated/agent-registry.md")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.check:
            ...
            print(f"PASS agent registry is current ({registry['stats']['totalAgents']} agents)")
            return 0
        write_registry(registry)
        print(f"PASS agent registry generated ({registry['stats']['totalAgents']} agents)")
        return 0
